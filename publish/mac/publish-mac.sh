...
ZIP_NAME="LianXiang-Browser-${VERSION}-macos-${ARCH}.zip"
APP_EXPORT="$OUTPUT_DIR/LianXiang-Browser-${VERSION}-macos-${ARCH}.app"
STAGE_DIR="$STAGING_ROOT/$TARGET"
APP_STAGE="$STAGE_DIR/链享浏览器.app"
...
if [[ "$SKIP_BUILD" -ne 1 ]]; then
  ...
  (
    cd "$ROOT_DIR"
    wails build -s -platform "darwin/$ARCH" -o lianxiang-browser
  )
else

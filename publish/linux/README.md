...
- `publish/output/LianXiang-Browser-<version>-linux-<arch>.tar.gz`
- `publish/output/lianxiang-browser_<version>_<arch>.deb`
...
- `.deb` installs app files under `/opt/lianxiang-browser`.
- `.deb` bundles `xray` and `sing-box` under `/opt/lianxiang-browser/bin`.
- Linux packages keep an empty `chrome/` placeholder with `README.md`, but do **not** bundle browser core binaries.
- `.deb` registers an application launcher at `/usr/share/applications/lianxiang-browser.desktop`.
- `.deb` installs standard Linux desktop icons under `/usr/share/icons/hicolor/*/apps/lianxiang-browser.png` and `/usr/share/pixmaps/lianxiang-browser.png`, so menus and launchers are more likely to pick up the app icon correctly.
- `.deb` bundles AppStream metadata under `/usr/share/metainfo/lianxiang-browser.metainfo.xml`, which improves recognition in software centers and GUI `.deb` installers.
- On Debian/Ubuntu desktop environments that already support local `.deb` GUI installers, the package can usually be installed by double-clicking it; if the host has no GUI installer association, use `sudo apt install ./lianxiang-browser_<version>_<arch>.deb`.

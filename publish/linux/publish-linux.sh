...
APP_BIN="$ROOT_DIR/build/bin/lianxiang-browser"
WAILS_CONFIG="$ROOT_DIR/wails.json"
CHROME_README_SRC="$ROOT_DIR/chrome/README.md"
APP_PACKAGE_NAME="lianxiang-browser"
APP_BINARY_NAME="lianxiang-browser"
APP_ICON_NAME="lianxiang-browser"
APP_DESKTOP_ID="lianxiang-browser.desktop"
APPSTREAM_ID="lianxiang-browser"
APP_NAME="链享浏览器"
...
if [[ "$SKIP_BUILD" -ne 1 ]]; then
  ...
  (
    cd "$ROOT_DIR"
    wails build -s -platform "linux/$ARCH" "${WAILS_BUILD_TAGS[@]}" -o lianxiang-browser
  )
else
...
cp "$APP_BIN" "$APP_STAGE/lianxiang-browser"
...
chmod +x "$APP_STAGE/lianxiang-browser" "$APP_STAGE/bin/xray" "$APP_STAGE/bin/sing-box"
...
mkdir -p "$OUTPUT_DIR"
TAR_NAME="LianXiang-Browser-${VERSION}-linux-${ARCH}.tar.gz"
tar -C "$APP_STAGE" -czf "$OUTPUT_DIR/$TAR_NAME" .

PKG_ROOT="$DEB_STAGE/${APP_PACKAGE_NAME}_${VERSION}_${ARCH}"
INSTALL_ROOT="$PKG_ROOT/opt/lianxiang-browser"
...
cp "$APP_STAGE/lianxiang-browser" "$INSTALL_ROOT/lianxiang-browser"
...
chmod +x "$INSTALL_ROOT/lianxiang-browser" "$INSTALL_ROOT/bin/xray" "$INSTALL_ROOT/bin/sing-box"
...
Name=${APP_NAME}
Comment=${APP_SUMMARY}
Exec=/opt/lianxiang-browser/${APP_BINARY_NAME}
TryExec=/opt/lianxiang-browser/${APP_BINARY_NAME}
Icon=${APP_ICON_NAME}
...
<component type="desktop-application">
  ...
  <description>
    <p>链享浏览器 manages isolated browser profiles, proxy binding, and local environment configuration for multi-account workflows.</p>
...
Description: ${APP_NAME} desktop app
 ...
 链享浏览器 manages isolated browser profiles, proxy binding, and
...
#!/bin/sh
set -e
ln -sf /opt/lianxiang-browser/lianxiang-browser /usr/bin/lianxiang-browser
chmod +x /opt/lianxiang-browser/lianxiang-browser /opt/lianxiang-browser/bin/xray /opt/lianxiang-browser/bin/sing-box || true
...
if [ "$1" = "remove" ] || [ "$1" = "purge" ]; then
  rm -f /usr/bin/lianxiang-browser
fi
if command -v update-desktop-database >/dev/null 2>&1; then
  update-desktop-database /usr/share/applications >/dev/null 2>&1 || true
fi

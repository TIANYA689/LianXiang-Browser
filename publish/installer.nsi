...
; Usage: makensis /DVERSION=1.4.5 /DSTAGINGDIR=C:\path\to\staging installer.nsi
...
!macro WriteCloseProcessScript HANDLE
  ...
  FileWrite ${HANDLE} "function Get-LianXiangBrowserProcesses {$\r$\n"
  ...
  FileWrite ${HANDLE} "  $$procs = Get-LianXiangBrowserProcesses$\r$\n"
  ...
  FileWrite ${HANDLE} "$$left = Get-LianXiangBrowserProcesses$\r$\n"
...
OutFile "..\publish\output\LianXiang-Browser-Setup-${VERSION}.exe"
...
!define MUI_DIRECTORYPAGE_TEXT_TOP "请选择 链享浏览器 的安装目录。建议安装到非 C 盘的可写目录，例如 D:\software\链享浏览器 或 E:\software\链享浏览器；不要安装到 C 盘、Program Files 或其他受权限保护的目录。"
...
!define MUI_FINISHPAGE_RUN_TEXT "Launch 链享浏览器"
...
Section "链享浏览器 (required)" SecMain
  SectionIn RO
...
!endif
  ...
  WriteRegStr HKLM "${UNINSTALL_KEY}" "Publisher"       "LianXiang Browser Team"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
  !insertmacro MUI_DESCRIPTION_TEXT ${SecMain}    "链享浏览器 main program and default config (required)"

---
name: lianxiang-browser-openclaw
description: Manually invoke when the user wants OpenClaw to work inside a local 链享浏览器 / lianxiang-browser instance through LaunchServer and remote CDP, including launching by launchCode, checking or switching the active runtime target, stopping an instance, or running a saved 链享浏览器 automation script.
when_to_use: Use this skill whenever the task mentions 链享浏览器, lianxiang-browser, LaunchServer, launchCode, profileId/profileName selectors, runtime active/status/stop, attaching OpenClaw to an already running browser via remote CDP, or executing a saved 链享浏览器 automation script such as news-query-txt. This workflow can launch, stop, or mutate browser state, so prefer explicit invocation instead of silent automatic use.
compatibility: Designed for OpenClaw with same-host 链享浏览器 LaunchServer access. Requires loopback access to the LaunchServer base URL plus either curl or PowerShell for host-side HTTP calls.
disable-model-invocation: true
argument-hint: "[goal-or-script]"
metadata: {"openclaw":{"skillKey":"lianxiang-browser-openclaw","primaryEnv":"LIANXIANG_BROWSER_API_KEY","requires":{"anyBins":["curl","pwsh","powershell"]}}}
---

# 链享浏览器 via OpenClaw

Invoke this skill explicitly when the user wants OpenClaw to control browser work that should run inside 链享浏览器 (`lianxiang-browser`) instead of a browser launched directly by OpenClaw.
...
- OpenClaw Gateway and 链享浏览器 run on the same machine.
- 链享浏览器 LaunchServer is reachable on loopback.
- OpenClaw Browser has a preconfigured remote CDP profile pointing at the 链享浏览器 LaunchServer URL.
...
- Base URL: `LIANXIANG_BROWSER_BASE_URL`, default `http://127.0.0.1:19876`
- API header: `LIANXIANG_BROWSER_API_HEADER`, default `X-LianXiang-Api-Key`
- API key: `LIANXIANG_BROWSER_API_KEY`, optional
- Browser profile name: assume `lianxiang-browser` unless the user tells you a different OpenClaw browser profile name
...
- Treat 链享浏览器 as the source of truth for profile state, proxies, tags, launch codes, and Chrome cores.
- Do not switch to another 链享浏览器 target while the current remote CDP session is still being used unless the user explicitly asks to switch. LaunchServer exposes one active CDP target at a time. Query `GET /api/runtime/active` before switching if the current target matters.
- Do not claim that OpenClaw `browser stop` stops the 链享浏览器 process. For remote CDP profiles it only detaches the OpenClaw control session.

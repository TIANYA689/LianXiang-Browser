# lianxiang-browser-openclaw 使用说明

## 1. 安装 Skill 方式

把 `skills/lianxiang-browser-openclaw` 整个目录复制到 OpenClaw 的 `skills` 目录。
...
如果 链享浏览器 开了 API Key，安装时补上 `-ApiKey` 或 `--api-key`。
...
如果脚本已经在 链享浏览器 里绑定了目标实例和默认参数，通常这两句就够了。

...
$effectiveApiHeader = Resolve-DefaultValue $ApiHeader "X-LianXiang-Api-Key"
$effectiveApiKey = Resolve-DefaultValue $ApiKey $env:LIANXIANG_BROWSER_API_KEY
if ([string]::IsNullOrWhiteSpace($ApiHeader) -and -not [string]::IsNullOrWhiteSpace($env:LIANXIANG_BROWSER_API_HEADER)) {
    $effectiveApiHeader = $env:LIANXIANG_BROWSER_API_HEADER.Trim()
}
if ([string]::IsNullOrWhiteSpace($BaseUrl) -and -not [string]::IsNullOrWhiteSpace($env:LIANXIANG_BROWSER_BASE_URL)) {
    $effectiveBaseUrl = $env:LIANXIANG_BROWSER_BASE_URL.Trim()
}

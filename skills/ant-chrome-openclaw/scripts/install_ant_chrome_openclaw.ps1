<#
.SYNOPSIS
Install the lianxiang-browser-openclaw skill into an OpenClaw skills directory and optionally merge openclaw.json.
...
param(
    ...
    [string]$ConfigFile = "",
    [string]$BrowserProfileName = "lianxiang-browser",
    [string]$BaseUrl = "",
    [string]$ApiHeader = "",
    [string]$ApiKey = "",
    [string]$Color = "#0F766E",
...
function Show-Usage {
    ...
    Write-Host "  -BrowserProfileName  Browser profile name to create or update (default: lianxiang-browser)"
    Write-Host "  -BaseUrl             链享浏览器 LaunchServer base URL"
...
$skillName = "lianxiang-browser-openclaw"
$effectiveBaseUrl = Resolve-DefaultValue $BaseUrl (Resolve-DefaultValue $env:LIANXIANG_BROWSER_BASE_URL "http://127.0.0.1:19876")
$effectiveApiHeader = Resolve-DefaultValue $ApiHeader (Resolve-DefaultValue $env:LIANXIANG_BROWSER_API_HEADER "X-LianXiang-Api-Key")
$effectiveApiKey = Resolve-DefaultValue $ApiKey (Resolve-DefaultValue $env:LIANXIANG_BROWSER_API_KEY "")
...
if (-not [string]::IsNullOrWhiteSpace($resolvedConfigFile)) {
    ...
    $envMap["LIANXIANG_BROWSER_BASE_URL"] = $effectiveBaseUrl
    $envMap["LIANXIANG_BROWSER_API_HEADER"] = $effectiveApiHeader

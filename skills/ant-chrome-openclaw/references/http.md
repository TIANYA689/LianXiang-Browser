...
```powershell
pwsh -File skills/lianxiang-browser-openclaw/scripts/invoke_ant_chrome_api.ps1 `
  -Method GET `
  -Path /api/health
```
...
```bash
BASE_URL="${LIANXIANG_BROWSER_BASE_URL:-http://127.0.0.1:19876}"
API_HEADER="${LIANXIANG_BROWSER_API_HEADER:-X-LianXiang-Api-Key}"

curl_ant() {
  if [ -n "${LIANXIANG_BROWSER_API_KEY:-}" ]; then
    curl -sS -H "${API_HEADER}: ${LIANXIANG_BROWSER_API_KEY}" "$@"
  else
    curl -sS "$@"
  fi
}
...
```powershell
$baseUrl = if ($env:LIANXIANG_BROWSER_BASE_URL) { $env:LIANXIANG_BROWSER_BASE_URL } else { "http://127.0.0.1:19876" }
$apiHeader = if ($env:LIANXIANG_BROWSER_API_HEADER) { $env:LIANXIANG_BROWSER_API_HEADER } else { "X-LianXiang-Api-Key" }
```
...
```powershell
$headers = @{}
if ($env:LIANXIANG_BROWSER_API_KEY) { $headers[$apiHeader] = $env:LIANXIANG_BROWSER_API_KEY }
...
Use this when the user wants to see what 链享浏览器 scripts already exist before choosing one to run.
...
- For scripts already bound to a target in 链享浏览器, prefer the minimal body and only override what the user explicitly asked to change.

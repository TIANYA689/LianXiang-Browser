# lianxiang-browser + OpenClaw 自动化整合方案
...
- 链享浏览器 负责实例选择、启动、停止、自动化执行

核心目标不是“让 OpenClaw 深度耦合 lianxiang-browser 内部实现”，而是让 lianxiang-browser 暴露稳定、通用、可复用的公共入口。

## 设计原则

- 低耦合：不默认做 `/api/automation/openclaw/*` 私有后端协议
- 公共入口优先：优先用通用 `runtime` / `automation/scripts` API
- 稳定性优先：把 `LaunchServer + CDP + Playwright` 复杂度尽量收进 lianxiang-browser
...
### 2. 想把自动化复杂度交给 lianxiang-browser
...
- 选择现成脚本
- 透传 `selector` / `params`
- 由 lianxiang-browser 本地执行 `playwright-cdp`
- 返回结果、日志、产物路径
...
先把 lianxiang-browser 做成稳定的通用浏览器执行端，再让 OpenClaw 调这些公共能力。

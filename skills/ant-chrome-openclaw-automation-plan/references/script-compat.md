...
2. 链享浏览器 读取 `scriptId`
3. 链享浏览器 调 `AutomationScriptRunWithOptions`
4. 再进入 `RunScriptTask`
5. 本地 runner 执行脚本
...
也就是把 LaunchServer + CDP + Playwright 的细节都收进 链享浏览器，不要让 OpenClaw 自己补：

1. OpenClaw 只给 `scriptId + selector + params`
2. 链享浏览器 在本地运行 `playwright-cdp` 脚本
...
如果 OpenClaw 想把 LaunchServer + Playwright + artifact 输出这些复杂度都交给 链享浏览器，本地直接执行现成脚本，优先走：

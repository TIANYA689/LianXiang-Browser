...
实例列表 -> 导入（Chrome 用户数据 / 链享实例包）
实例列表 -> 选中实例 -> 导出
默认书签 -> 导入书签（Chrome / Edge / Firefox HTML）-> 保存
\`\`\`
...
导出：实例列表 -> 勾选实例 -> 导出
单个导出：实例列表 -> 更多 -> 导出
导入实例包：实例列表 -> 导入 -> 链享实例包 -> 选择 ZIP
导入 Chrome：实例列表 -> 导入 -> Chrome 用户数据目录 -> 选择完整的 --user-data-dir
\`\`\`
...
- Chrome 导入前必须关闭使用源目录的全部 Chrome 窗口
- Chrome 导入应选择包含 Local State 的完整目录，不要只选择 Default
- Chrome 数据会复制为独立实例；修改其指纹时会提示登录态和网站风控风险

## 默认书签导入与分组

\`\`\`text
导入：默认书签 -> 导入书签 -> 选择浏览器导出的 HTML -> 保存
手动分组：默认书签 -> 添加分组 -> 输入分组名 -> 添加书签 -> 保存
多级目录：分组名称使用 工作/AI 形式
同步旧实例：保存后自动增量同步，或点击手动同步
\`\`\`

- 导入会保留 HTML 中的目录结构，并按 URL 跳过重复书签
- 同步只追加缺失书签，不会删除、改名或移动实例已有书签
- 运行中的实例会跳过，停止后可再次手动同步
...
1. 链享浏览器 和 OpenClaw 在同一台机器上
2. 链享浏览器 的 LaunchServer 可访问，默认是 \`http://127.0.0.1:19876\`
3. OpenClaw 已安装 \`lianxiang-browser-openclaw\` skill，并且已有指向 链享浏览器 的远程 CDP 浏览器配置
...
\`\`\`powershell
pwsh -File skills/lianxiang-browser-openclaw/scripts/install_ant_chrome_openclaw.ps1 -SetDefaultProfile
\`\`\`

如果没探测到 OpenClaw 路径，再用：

\`\`\`powershell
pwsh -File skills/lianxiang-browser-openclaw/scripts/install_ant_chrome_openclaw.ps1 -TargetSkillsDir "C:\\path\\to\\openclaw\\skills" -ConfigFile "C:\\path\\to\\openclaw\\openclaw.json" -SetDefaultProfile
\`\`\`
...
如果 链享浏览器 开了 API Key，安装时补上对应参数即可。
...
\`\`\`text
使用 lianxiang-browser-openclaw skill。
\`\`\`
...
\`\`\`bash
curl -H "X-LianXiang-Api-Key: <your-api-key>" http://127.0.0.1:19876/api/health
\`\`\`

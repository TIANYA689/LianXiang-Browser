...
function Build-WindowsBinary {
    ...
    try {
        ...
        try {
            ...
            Invoke-NativeCommand -FilePath "npm.cmd" -Arguments @("ci", "--prefer-offline", "--no-audit", "--no-fund")
    ...
    if (-not (Test-Path -LiteralPath $binaryPath -PathType Leaf)) {
        throw "构建产物不存在: build\bin\lianxiang-browser.exe"
    }
    Write-Host "✓ 构建成功: build\bin\lianxiang-browser.exe"
    Write-Host ""
}
...
function New-WindowsStaging {
    ...
    Copy-Item -LiteralPath $binaryPath -Destination (Join-Path $stagingDir "lianxiang-browser.exe") -Force
    if (-not (Test-Path -LiteralPath (Join-Path $stagingDir "lianxiang-browser.exe") -PathType Leaf)) {
        throw "staging 中缺少 lianxiang-browser.exe"
    }
    Write-Host "✓ 复制 lianxiang-browser.exe"
...
function New-WindowsPortableArchive {
    ...
    $archiveName = "LianXiang-Browser-$script:ResolvedVersion-windows-amd64-portable.zip"
    $archivePath = Join-Path $outputDir $archiveName
    $rootName = "LianXiang-Browser-$script:ResolvedVersion-windows-amd64-portable"
...
function Publish-Linux {
    ...
    $archOutFile = Join-Path $env:TEMP ("lianxiang-browser-linux-arch-" + [guid]::NewGuid().ToString("N") + ".txt")
...
try {
    ...
    if ($script:WindowsInstallerDone) {
        Write-Host "Windows 安装包: publish\output\LianXiang-Browser-Setup-$script:ResolvedVersion.exe"
    }
